...
public class VideoThumbnailLoader {
        ...
        protected Bitmap doInBackground(Void... params) {
            ...
            if (!TextUtils.isEmpty(url)) {
                ...
                if (bitmap == null||bitmap.isRecycled()) {
                    File file = new FileUtil().createSDFile(FinalVarible.PIC_PATH + filename);//创建文件,这里由于项目原因,我就随便写一个,实际情况不是这样,大家留意一下
                    if (null != file && file.exists()) {//去磁盘缓存取
                        ...
                        if (null==bitmap) {
                            bitmap = getVideoThumbnail(url, width, height, MediaStore.Video.Thumbnails.MICRO_KIND);
                            //将图片保存到磁盘文件,作为缓存
                            BitmapUtils.saveBitmapToFile(bitmap, FinalVarible.PIC_PATH, filename, Bitmap.CompressFormat.PNG);
                        }
     ...
     *   mHolder.ivBg.setTag(imgUrl);  //这里注意,我们给imageview设置了一个Tag,后面自有妙用
         VideoThumbnailLoader.getIns().display(url, mHolder.ivBg,mImgWidth,mImgHeight, new VideoThumbnailLoader.ThumbnailListener() {
              @Override
